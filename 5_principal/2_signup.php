...
<html>
    <head>
        ...
        <title>Principal's signup</title>
    </head>
    <body>
		...
		{		
			...
			$pidquery="SELECT * FROM principal WHERE pid='$pid'";
			$query=mysqli_query($conn,$pidquery);
			...
			{    
				...
				$_SESSION['s_pid']=$details['pid'];
				$_SESSION['s_pname']=$details['pname'];
				...
				if(empty($passchk))
				{
					if($pass===$cpass)
					{
						$add="UPDATE principal SET pass='$pw' WHERE pid='$pid'";
				...
				else
				{
					...
					<div class="error">
						<p><span><ion-icon name="alert-circle"></ion-icon></span>
							WARNING!&nbsp Already&nbsp Registered&nbsp Account</p>
					</div>
					<?php
				}				
			}	
